...
{
struct test_data *td; td = malloc(sizeof(struct test_data));
(*td).user_id = id;
...
<<<<<<< HEAD
int res; res = lfds711_stack_pop(&mystack, &se);
if (res == 0)
>>>>>>> origin/main
=======
int res; res = lfds711_stack_pop((struct lfds711_stack_state *) s, &se);
if (res != 0)
>>>>>>> origin/scorso
        {
free((*se).value);
...
=======
int res; res = 1;
        int data_structure_size; data_structure_size = 0;
...
data = (*se).value;
unsigned long long int id_found; id_found = (*data).user_id;
ids[(*data).user_id] = 1;
free(data);
        }
return data_structure_size;
