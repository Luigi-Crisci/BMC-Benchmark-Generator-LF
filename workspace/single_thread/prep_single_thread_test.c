...
{
  struct lfds711_btree_au_element __attribute__((aligned(8))) * volatile left,
      *volatile right,
      *volatile up;
  void __attribute__((aligned(8))) * volatile value;
  void
      *key;
};
struct lfds711_btree_au_state
{
  struct lfds711_btree_au_element __attribute__((aligned(8))) * volatile root;
...
{
  struct lfds711_freelist_element __attribute__((aligned(128))) * volatile top[2];
...
{
  struct lfds711_list_aso_element __attribute__((aligned(8))) * volatile next;
  void __attribute__((aligned(8))) * volatile value;
  void
      *key;
};
struct lfds711_list_aso_state
{
  ...
  dummy_element;
  struct lfds711_list_aso_element __attribute__((aligned(128))) * start;
  int (*key_compare_function)(void const *new_key, void const *existing_key);
  enum lfds711_list_aso_existing_key
      existing_key;
  void
...
{
  struct lfds711_list_asu_element __attribute__((aligned(8))) * volatile next;
  void __attribute__((aligned(8))) * volatile value;
  void
      *key;
};
struct lfds711_list_asu_state
{
  ...
  dummy_element;
  struct lfds711_list_asu_element __attribute__((aligned(128))) * volatile end;
  struct lfds711_list_asu_element __attribute__((aligned(128))) * start;
  void
      *user_state;
  struct lfds711_misc_backoff_state
      after_backoff,
...
{
  lfds711_pal_uint_t volatile sequence_number;
  void
      *volatile key,
      *volatile value;
};
...
struct lfds711_queue_bss_state
{
  lfds711_pal_uint_t
      number_elements,
      mask;
  lfds711_pal_uint_t volatile read_index,
      write_index;
  struct lfds711_queue_bss_element
      *element_array;
  void
...
{
  struct lfds711_queue_umm_element __attribute__((aligned(16))) * volatile next[2];
  void
      *key,
      *value;
};
...
{
  struct lfds711_queue_umm_element __attribute__((aligned(128))) * volatile enqueue[2],
      *volatile dequeue[2];
  lfds711_pal_uint_t volatile __attribute__((aligned(128)))
  aba_counter;
  void __attribute__((aligned(128))) * user_state;
  struct lfds711_misc_backoff_state
      dequeue_backoff,
      enqueue_backoff;
};
...
{
  struct lfds711_stack_element __attribute__((aligned(128))) * volatile top[2];
  void __attribute__((aligned(128))) * user_state;
  struct lfds711_misc_backoff_state
      pop_backoff,
      push_backoff;
...
/* Commentata in quanto non più utilizzata e sostituta dall'if precedente
    {
      ...
      __asm__ __volatile__("lock;"
                           "cmpxchg16b %0;"
                           "setz       %4;"
                           : "+m"((ss->top)[0]), "+m"((ss->top)[1]), "+a"((original_top)[0]), "+d"((original_top)[1]), "=q"(result)
  ...
  *se = original_top[0];
  {
    if ((backoff_iteration) < 2)
      (ss->pop_backoff).backoff_iteration_frequency_counters[(backoff_iteration)]++;
    if (++(ss->pop_backoff).total_operations >= 10000 && (ss->pop_backoff).lock == LFDS711_MISC_FLAG_LOWERED)
    {
      char unsigned result;
      lfds711_pal_uint_t __attribute__((aligned(128))) compare = LFDS711_MISC_FLAG_LOWERED;
      {
        result = (char unsigned)__atomic_compare_exchange_n(&(ss->pop_backoff).lock, &compare, LFDS711_MISC_FLAG_RAISED, LFDS711_MISC_CAS_STRENGTH_WEAK, 0, 0);
      };
      if (result == 1)
      {
        if ((ss->pop_backoff).backoff_iteration_frequency_counters[1] < (ss->pop_backoff).backoff_iteration_frequency_counters[0] / 100)
        {
          if ((ss->pop_backoff).metric >= 11)
            (ss->pop_backoff).metric -= 10;
        }
        else
          (ss->pop_backoff).metric += 10;
        (ss->pop_backoff).backoff_iteration_frequency_counters[0] = 0;
        (ss->pop_backoff).backoff_iteration_frequency_counters[1] = 0;
        (ss->pop_backoff).total_operations = 0;
        __atomic_thread_fence(3);
        {
          (void)__atomic_exchange_n((&(ss->pop_backoff).lock), (LFDS711_MISC_FLAG_LOWERED), 0);
        };
      }
    }
  };
                        ...
                        struct lfds711_stack_element *se)
{
  ...
  {
    if ((backoff_iteration) < 2)
      (ss->push_backoff).backoff_iteration_frequency_counters[(backoff_iteration)]++;
    if (++(ss->push_backoff).total_operations >= 10000 && (ss->push_backoff).lock == LFDS711_MISC_FLAG_LOWERED)
    {
      char unsigned result;
      lfds711_pal_uint_t __attribute__((aligned(128))) compare = LFDS711_MISC_FLAG_LOWERED;
      {
        result = (char unsigned)__atomic_compare_exchange_n(&(ss->push_backoff).lock, &compare, LFDS711_MISC_FLAG_RAISED, LFDS711_MISC_CAS_STRENGTH_WEAK, 0, 0);
      };
      if (result == 1)
      {
        if ((ss->push_backoff).backoff_iteration_frequency_counters[1] < (ss->push_backoff).backoff_iteration_frequency_counters[0] / 100)
        {
          if ((ss->push_backoff).metric >= 11)
            (ss->push_backoff).metric -= 10;
        }
        else
          (ss->push_backoff).metric += 10;
        (ss->push_backoff).backoff_iteration_frequency_counters[0] = 0;
        (ss->push_backoff).backoff_iteration_frequency_counters[1] = 0;
        (ss->push_backoff).total_operations = 0;
        __atomic_thread_fence(3);
        {
          (void)__atomic_exchange_n((&(ss->push_backoff).lock), (LFDS711_MISC_FLAG_LOWERED), 0);
        };
      }
    }
  };
