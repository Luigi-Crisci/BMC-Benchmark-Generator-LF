...
int main(){
 ...
 printf("Result %d : %d\n",result->x,result->y);
 int s = result->x;
 ((void) sizeof ((s == 6) ? 1 : 0), __extension__ ({ if (s == 6) ; else __assert_fail ("s == 6", "single_thread_test.c", 50, __extension__ __PRETTY_FUNCTION__); }));
 return 0;
}
